  ...
  onMount(() => {
    ...
    (async () => {
      ...
      diagram.linkTemplate = new go.Link({
        ...
        // Линия связи - всегда видна, меняет цвет при выборе
        new go.Shape({
          strokeWidth: 2
        })
          .themeData('stroke', '', null, (data, obj) => {
            // Если связь выбрана - красная, иначе стандартный цвет темы
            return obj.part?.isSelected ? null : 'linkStroke';
          })
          .bindObject('stroke', 'isSelected', (sel) => sel ? 'red' : null),
        // Стрелка в начале (маленький треугольник)
        new go.Shape({
          fromArrow: 'Triangle',
          scale: 0.8,
          stroke: null
        })
          .themeData('fill', '', null, (data, obj) => {
            return obj.part?.isSelected ? null : 'linkStroke';
          })
          .bindObject('fill', 'isSelected', (sel) => sel ? 'red' : null),
        // Стрелка в конце (показывает направление)
        new go.Shape({
          toArrow: 'Standard',
          scale: 1.2,
          stroke: null
